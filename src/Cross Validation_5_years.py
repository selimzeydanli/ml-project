...
def cross_validate_lstm(X, y, n_splits=5):
    ...
    return np.mean(r2_scores), np.mean(mae_scores), scaler_X, scaler_y

# Function to process variable scenario (updated)
def process_variable_scenario(df, scenario_name):
    ...
    r2, mae, scaler_X, scaler_y = cross_validate_lstm(X, y)

    print(f"\n{scenario_name} Scenario (Cross-validation results):")
    print(f"Mean R-squared: {r2:.4f}")
    ...
    final_model.fit(X_scaled, y_scaled, epochs=1, batch_size=32, verbose=0)
...
data_dir = r"C:\Users\Selim\Desktop\ml-project\data\TransactionDatabases_lstm_5_years"
