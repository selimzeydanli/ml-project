...
from src.utils import (
    get_order_dbs_lstm_5_years_dir,
    read_json_to_dataframe,
    get_truck_dbs_lstm_5_years_dir,
    empty_folder,
    get_transaction_dbs_lstm_5_years_dir,
)


empty_folder(get_transaction_dbs_lstm_5_years_dir())
...
while orderDate <= endLoopDate:
    ...
    order_df = read_json_to_dataframe(
        os.path.join(
            get_order_dbs_lstm_5_years_dir(), f"OrderDatabase_lstm_5_years-{orderDateStr}.json"
        )
    )
    truck_1_df = read_json_to_dataframe(
        os.path.join(
            get_truck_dbs_lstm_5_years_dir(), f"TruckDatabase_lstm_5_years_{truckDateFirstStr}.json"
        )
    )
    truck_2_df = read_json_to_dataframe(
        os.path.join(
            get_truck_dbs_lstm_5_years_dir(), f"TruckDatabase_lstm_5_years_{truckDateSecondStr}.json"
        )
    )
